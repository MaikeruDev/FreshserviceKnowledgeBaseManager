...
export class FreshserviceClient {
  ...
  async request(method, apiPath, { query, body, form, retry = 1 } = {}) {
    ...
    if (!res.ok) {
      const errorList = Array.isArray(json?.errors)
        ? json.errors.map((e) => `${e.field ? e.field + ": " : ""}${e.message}`).join("; ")
        : "";
      const detail =
        [json?.description || json?.message, errorList].filter(Boolean).join(" – ") ||
        text?.slice(0, 300) ||
        res.statusText;
  ...
  /** Active agents (id, first_name, last_name, email, ...). Used for the "who are you" / author picker. */
  agents() {
    return this.listAll("/agents", "agents", { active: true });
  }

  /** The agent who owns this API key (= the author Freshservice records for every article created with it).
   *  /agents/me is not in the official docs but supported by Freshworks products; null if unavailable. */
  async me() {
    try {
      const { json } = await this.request("GET", "/agents/me");
  ...
  /**
   * Download an attachment via the documented API endpoint (GET /api/v2/attachments/[id], works for Solution Article
   * attachments with API-key auth; redirects to storage). Returns the fetch Response.
   */
  fetchAttachmentApi(attachmentId) {
    const url = new URL(`/api/v2/attachments/${encodeURIComponent(attachmentId)}`, this.baseUrl);
    if (this.workspaceId) url.searchParams.set("workspace_id", this.workspaceId);
    return fetch(url, { headers: { Authorization: this.authHeader, Accept: "*/*" }, redirect: "follow" });
  }

  /** Fetch an attachment via the helpdesk web URL (needs a browser session on most instances; kept as last resort). */
